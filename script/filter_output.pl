...
my $ends_for = get_ends($ends_file);
...
if ( !$keep_ends_without_rnaseq ) {
    $regions = remove_ends_without_rnaseq( $regions, $ends_for );
}
...
sub get_regions {
    my ( $file, $analysis ) = @_;    ## no critic (ProhibitReusedNames)
    ...
    return DETCT::Misc::Output::parse_table(
        {
            analysis     => $analysis,
            table_file   => $file,
