...
# PODNAME: run_downsample_pipeline.pl
# ABSTRACT: Run DETCT downsampling pipeline
...
# Create analysis
my $analysis = DETCT::Analysis::Downsample->new_from_yaml($analysis_yaml);

# Create pipeline
my $pipeline = DETCT::Pipeline::Downsample->new(
    {
        scheduler    => $scheduler,
